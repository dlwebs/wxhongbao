var jsmember = new function() {
	...
	this.act_pwd = function() {
		...
		if("act_pwd" in this.html) {
			...
			kj.ajax.get(kj.cfg('baseurl') + "/index.php?app=member&app_act=pwd" , function(data) {
	...
	this.act_default = function() {
		jsheader.close();
		kj.dialog({'url':kj.cfg('baseurl') + '/index.php?app=member','id':'member','type':'iframe','title':'我的订单','w':700,'max_h':650,'showbtnmax':false,'showbtnhide':false});
	}
	this.act_myvip = function() {
		jsheader.close();
		kj.dialog({'url':kj.cfg('baseurl') + '/index.php?app=member&app_act=myvip','id':'membermyvip','type':'iframe','title':'我的经验','w':750,'max_h':650,'showbtnmax':false,'showbtnhide':false});
	}
	this.act_myintegral = function() {
		jsheader.close();
		kj.dialog({'url':kj.cfg('baseurl') + '/index.php?app=member&app_act=myintegral','id':'membermymyintegral','type':'iframe','title':'我的积分','w':750,'max_h':650,'showbtnmax':false,'showbtnhide':false});
	}
	this.act_msg = function() {
		jsheader.close();
		kj.dialog({'url':kj.cfg('baseurl') + '/index.php?app=member&app_act=msg','id':'membermymymsg','type':'iframe','title':'我的留言','w':720,'max_h':650,'showbtnmax':false,'showbtnhide':false});
	}
	this.act_repayment = function() {
		jsheader.close();
		kj.dialog({'url':kj.cfg('baseurl') + '/index.php?app=member&app_act=repayment','id':'membermymyrepayment','type':'iframe','title':'我的预付款','w':720,'max_h':650,'showbtnmax':false,'showbtnhide':false});
	}
	this.open = function(url,title,w,h) {
		if(!w) w = 720;
		var data = {'url':url,'id':'openwin','type':'iframe','title':title,'showbtnmax':false,'showbtnhide':false};
		if(w) data.w = w;
		if(h) data.max_h = h;
		kj.dialog(data);
	}
